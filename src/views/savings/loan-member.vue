<template>
  <MainLayout>
    <h1 class="text-2xl font-bold text-primary mb-4" data-aos="fade-right">
      {{ $t('dashboard.totalLoanMembers') }} ({{ data.length }})
    </h1>
    ...
    <table class="w-full border border-collapse text-left" data-aos="fade-up" data-aos-delay="150">
      ...
      <tbody class="capitalize">
        <tr v-for="(item, index) in data" :key="item.AMCode">
        ...
        <tr v-if="!loading && !data.length">
          <td colspan="6" class="px-4 py-6 border text-center text-gray-500">{{ $t('common.noData') }}</td>
        </tr>
      </tbody>
    </table>
