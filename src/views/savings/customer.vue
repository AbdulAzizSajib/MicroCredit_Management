<template>
  <MainLayout>
    ...
    <a-modal
      v-model:open="isAddCollectionModalVisible"
      :title="$t('collection.addCollection')"
      @cancel="isAddCollectionModalVisible = false"
      :footer="null"
      width="550px"
    >
      <form @submit.prevent="createCollection">
        <div class="space-y-4 mb-6">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('collection.customerName') }}</label>
            <a-input :value="`${collectionForm.CustomerName} (${collectionForm.CustomerCode})`" disabled />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('customer.amCode') }}</label>
            <a-input :value="collectionForm.AMCode" disabled :placeholder="$t('common.autoGenerated')" />
          </div>
          <div v-if="collectionCustomerInfo" class="grid grid-cols-3 gap-2 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border">
            <div class="text-center">
              <div class="text-[10px] uppercase text-gray-500 font-semibold">{{ $t('collection.totalAmount') }}</div>
              <div class="text-sm font-bold text-blue-700">{{ Number(collectionCustomerInfo.TotalAmount || 0).toFixed(2) }}</div>
            </div>
            <div class="text-center border-x">
              <div class="text-[10px] uppercase text-gray-500 font-semibold">{{ $t('collection.receivable') }}</div>
              <div class="text-sm font-bold text-green-700">{{ Number(collectionCustomerInfo.TotalGivenAmount || 0).toFixed(2) }}</div>
            </div>
            <div class="text-center">
              <div class="text-[10px] uppercase text-gray-500 font-semibold">{{ $t('collection.dueAmount') }}</div>
              <div class="text-sm font-bold text-red-600">{{ Number(collectionCustomerInfo.DueAmount || 0).toFixed(2) }}</div>
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('common.period') }} <span class="text-red-500">*</span></label>
            <a-week-picker
              v-if="collectionForm.CollectionType === 'W'"
              class="w-full"
              :placeholder="$t('common.period')"
              value-format="YYYYWW"
              format="[Week] WW, YYYY"
              v-model:value="collectionForm.Period"
            />
            <a-month-picker
              v-else
              class="w-full"
              ...
              value-format="YYYYMM"
              format="MMM YYYY"
              :disabled-date="disabledCollectionPeriod"
              v-model:value="collectionForm.Period"
            />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('common.amount') }} <span class="text-red-500">*</span></label>
            <a-input-number class="w-full" v-model:value="collectionForm.Amount" :placeholder="$t('common.amount')" :precision="2" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('common.remarks') }} <span class="text-red-500">*</span></label>
            <a-textarea v-model:value="collectionForm.Remarks" :placeholder="$t('common.remarks')" :rows="3" />
          </div>
        </div>
        <div class="flex items-center gap-3">
          <button type="button" @click="isAddCollectionModalVisible = false"
            class="px-6 py-2 rounded font-semibold bg-gray-500 text-white hover:bg-gray-600 transition-colors">
            {{ $t('common.close') }}
          </button>
          <button type="submit" :disabled="isCollectionSaving"
            class="px-6 py-2 rounded font-semibold bg-green-600 text-white hover:bg-green-700 transition-colors">
            {{ isCollectionSaving ? $t('common.saving') : $t('common.save') }}
          </button>
        </div>
      </form>
    </a-modal>
...
const collectionCustomerInfo = ref(null);
const collectionPaidPeriods = ref([]);
const collectionMinPaidPeriod = ref(null);

const collectionDefaultForm = {
  CustomerCode: "",
  CustomerName: "",
  AMCode: "",
  CollectionType: "M",
...
const collectionForm = ref({ ...collectionDefaultForm });

const disabledCollectionPeriod = (current) => {
  if (!current) return false;
  if (!collectionMinPaidPeriod.value) return false;
  const p = `${current.year()}${String(current.month() + 1).padStart(2, "0")}`;
  if (p < collectionMinPaidPeriod.value) return true;
  if (collectionPaidPeriods.value.includes(p)) return true;
  return false;
};

const openAddCollectionModal = async (data) => {
  collectionForm.value = {
    ...collectionDefaultForm,
    CustomerCode: data.CustomerCode || "",
    CustomerName: data.CustomerName || "",
    Amount: Number(data.SavingAmount) || null,
  };
  collectionCustomerInfo.value = null;
  collectionPaidPeriods.value = [];
  collectionMinPaidPeriod.value = null;
  ...
  try {
    const mRes = await axios.get(`${apiBase}/customer/${code}/missing-installment`, getToken());
    const periods = (mRes?.data?.data || []).map(x => String(x.PaidPeriod));
    collectionPaidPeriods.value = periods;
    collectionMinPaidPeriod.value = periods.length ? periods.sort()[0] : null;
  } catch (e) { collectionPaidPeriods.value = []; collectionMinPaidPeriod.value = null; }
...
const createCollection = async () => {
  if (!collectionForm.value.CustomerCode?.trim()) return showNotification("error", "Member is required.");
  if (!collectionForm.value.Period) return showNotification("error", "Period is required.");
  if (!collectionForm.value.Amount || collectionForm.value.Amount <= 0) return showNotification("error", "Amount is required.");
  if (!collectionForm.value.Remarks?.trim()) return showNotification("error", "Remarks is required.");

  isCollectionSaving.value = true;
  try {
    const payload = { ...collectionForm.value };
    payload.Date = dayjs().format("YYYY-MM-DD");
    payload.Narration = payload.Remarks;
    const res = await axios.post(`${apiBase}/member-collection`, payload, getToken());
    showNotification("success", res?.data?.message || "Collection added successfully!");
    isAddCollectionModalVisible.value = false;
  } catch (error) {
    showNotification("error", error?.response?.data?.message || "Failed to add collection.");
  } finally {
