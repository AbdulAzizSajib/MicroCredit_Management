  ...
  <MainLayout>
    <div class="max-w-7xl mx-auto">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-indigo-700">Total Saving</h1>
        <a-button @click="$router.back()">Back</a-button>
      </div>

      <div class="text-right mb-3 flex justify-end items-center gap-2">
        <label class="font-semibold text-gray-700">Total Saving</label>
        <input
          type="text"
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await axios.get(`${apiBase}/dashboard/total-saving`, getToken());
    rows.value = res?.data?.data || [];
    totalSaving.value = res?.data?.total_saving ?? 0;
  } catch (error) {
    console.error(error);
    showNotification("error", "Failed to fetch total saving.");
  } finally {
    loading.value = false;
  }
};
