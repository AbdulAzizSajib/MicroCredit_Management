<template>
  <MainLayout>
    ...
    <table class="w-full border border-collapse text-left mt-5">
      <thead>
        <tr class="bg-primary text-white">
          ...
          <th class="border border-white px-4 py-2">{{ $t('common.invoiceNo') }}</th>
    ...
    <a-modal
      ...
      :title="$t('voucher.addMemberSavingsVoucher')"
      :footer="false"
      :width="1000"
      :maskClosable="false"
    >
        ...
        <a-form>
          ...
          <div class="p-2 pt-0 bg-gray-50">
            ...
            >
              <div class="w-[280px]">
                <label class="block text-sm font-medium text-gray-700"
                  >{{ $t('voucher.chequeNo') }}</label
                >
                <input
                  v-model="chequeDetails.ChequeNo"
                  type="text"
                  class="w-full border rounded-lg p-1 focus:outline-indigo-500"
                  :placeholder="$t('voucher.enterChequeNo')"
                />
              </div>

              <div class="w-[280px]">
                <label class="block text-sm font-medium text-gray-700"
                  >{{ $t('voucher.chequeName') }}</label
                >
                <input
...
const handleAddSale = () => {
  ...
  const initialCreditEntries = currentBatchInvoices.map((invNo) => {
    ...
    return {
      AccountCode: invoice?.AMCode || "",  // ✅ প্রতিটা invoice এর নিজের AMCode
...
const saveSaleVoucher = async () => {
  ...
  try {
    ...
    // ✅ প্রতিটা CollectionID এর সাথে তার AMCode map করুন
    const collectionEntries = checkedInvoice.value.map((id) => {
      const entry = creditVoucherEntries.value.find(
        (e) => String(e.BillNo) === String(id)
      );
      return {
        CollectionID: Number(id),
        AMCode: entry?.AccountCode || "",
      };
    });

    // ✅ checkedInvoice এর first ID অনুযায়ী first AMCode নিন
    const firstEntry = creditVoucherEntries.value.find(
      (e) => String(e.BillNo) === String(checkedInvoice.value[0])
    );
