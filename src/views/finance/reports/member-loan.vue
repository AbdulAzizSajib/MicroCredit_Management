    ...
    <table class="w-full border border-collapse text-left mt-5">
      <thead>
        <tr class="bg-primary text-white">
          ...
          <th class="border border-white px-4 py-2">Payment ID</th>
      ...
      <tbody class="capitalize">
        ...
        <template v-else v-for="data in paginatedData" :key="data.InvoiceNo">
          <tr>
            ...
            <td class="px-4 border">{{ data?.PayLoanPaymentID }}</td>
            ...
            <table class="w-full text-left">
              ...
              <tbody class="capitalize">
                ...
                <tr v-for="item in creditVoucherEntries" :key="'confirmed-' + item.BillNo" class="bg-gray-100 hover:bg-gray-200">
                  ...
                  <td class="px-4 border">{{ item.PayLoanPaymentID }}</td>
...
const fetchAllData = async () => {
  try {
    ...
    allData.value = (res?.data || []).map((item, idx) => ({
      ...
      LoanID: item.LoanID,
      PayLoanPaymentID: item.PayLoanPaymentID || "",
...
const handleAddSale = () => {
  ...
  const initialCreditEntries = currentBatchInvoices.map((invNo) => {
    ...
    return {
      ...
      PayLoanPaymentID: invoice?.PayLoanPaymentID || "",
    };
  });
...
const addVoucherEntry = () => {
  ...
  const newCreditEntries = modalForm.value.Details.map((detail) => ({
    ...
    PayLoanPaymentID: detail.PayLoanPaymentID || "",
...
const saveLoanVoucher = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      Narration: commonNarration.value,
      PayLoanPaymentEntries: selectedItems.map((item) => ({
        PayLoanPaymentID: Number(item.PayLoanPaymentID) || 0,
        AMCode: item.AMCode || item.LoanAMCode || "",
        PaymentDate: item.PaymentDate || "",
      })),
      PayLoanPaymentIDs: selectedItems
        .map((item) => Number(item.PayLoanPaymentID))
        .filter((id) => id > 0),
    };
