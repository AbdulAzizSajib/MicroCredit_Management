    ...
    <table class="w-full border border-collapse text-left">
      ...
      <tbody class="capitalize">
        <template v-for="data in allData" :key="data?.ChequeBookNo">
          <tr
            :class="{
              'bg-yellow-300': expandedRows[data?.ChequeBookNo],
            }"
          >
            <td class="px-4 border">
              <button
                ...
                :class="{
                  'rotate-45 ': expandedRows[data?.ChequeBookNo],
                  ' text-green-500': !expandedRows[data?.ChequeBookNo],
                }"
              >
                +
              </button>
            ...
            <td class="px-4 border text-right">{{ data?.NoOfBooks }}</td>
                        ...
                        <table class="w-full border-collapse bg-white">
                          <thead>
                            <tr class="bg-gray-100">
                              ...
                              <!-- <th
                                class="border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 w-16 text-center"
                              >
                                Actions
                              </th> -->
                            </tr>
                          ...
                          <tbody>
                            ...
                            >
                              ...
                              <!-- <td
                                class="border border-gray-300 px-3 py-2 flex justify-center"
                              >
                                <Icon
...
// New refs for collapse functionality
const expandedRows = ref({});
const activeKeys = ref({});
const chequeDetails = ref({});
const loadingBookNo = ref(null);
...
const countByStatus = (details, status) => {
  return details.filter((d) => d.Status === status).length;
};

// Toggle details expansion
const toggleDetails = async (ChequeBookNo) => {
  // Toggle the expanded state
  expandedRows.value[ChequeBookNo] = !expandedRows.value[ChequeBookNo];

  if (expandedRows.value[ChequeBookNo]) {
    // Set active key for collapse to open it automatically
    activeKeys.value[ChequeBookNo] = [ChequeBookNo];

    // Load details if not already loaded
    if (!chequeDetails.value[ChequeBookNo]) {
      await handleDetails(ChequeBookNo);
    }
  } else {
    // Clear active key to close collapse
    activeKeys.value[ChequeBookNo] = [];
  }
};
...
const handleDetails = async (ChequeBookNo) => {
  try {
    loadingBookNo.value = ChequeBookNo;
    ...
    if (res?.data.success === true) {
      chequeDetails.value[ChequeBookNo] = res?.data?.data?.details || [];
      chequeBookNoDetails.value = res?.data?.data;

      console.log("chequeDetails.value--------", chequeDetails.value);
      console.log(
        "chequeBookNoDetails.value--------",
        chequeBookNoDetails.value
      );
    } else {
      message.error(res?.data?.message || "Failed to load details");
      chequeDetails.value[ChequeBookNo] = [];
    }
  } catch (error) {
    console.log(error);
    chequeDetails.value[ChequeBookNo] = [];
    isDetailsLoading.value = false;
    loadingBookNo.value = null;
  }
};
