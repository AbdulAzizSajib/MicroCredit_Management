<template>
  <MainLayout>
    <div class="space-y-6">
      <h1 class="text-2xl font-bold text-primary" data-aos="fade-right">{{ $t('dashboard.totalShouldPay') }}</h1>
      ...
      <table v-else class="w-full border border-collapse text-left" data-aos="fade-up" data-aos-delay="150">
        <thead>
          <tr class="bg-primary text-white">
            ...
            <th class="border border-white px-4 py-2 text-right">{{ $t('dashboard.totalShouldPay') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, i) in allData" :key="i" class="hover:bg-gray-50">
            ...
            <td class="px-4 border text-right">{{ formatAmount(Number(item.ShouldPay || 0)) }}</td>
          </tr>
