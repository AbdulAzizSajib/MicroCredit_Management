  ...
  <MainLayout>
    <div class="max-w-7xl mx-auto space-y-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h1 class="text-2xl font-bold text-indigo-700">Total Loan Due</h1>
      ...
      <div class="text-right mb-3 flex justify-end items-center gap-2">
        <label class="font-semibold text-gray-700">Total Loan Due</label>
        <input type="text" class="w-36 border rounded-lg p-1 text-right bg-yellow-300 text-black font-bold"
          :value="formatAmount(totalLoanDue)" readonly />
      </div>
      ...
      <table v-else class="w-full border border-collapse text-left">
        <thead>
          <tr class="bg-primary text-white">
            ...
            <th class="border border-white px-4 py-2 text-right">Total Loan Due</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-for="(item, index) in rows" :key="`${item.MemberCode}-${index}`">
            ...
            <td class="px-4 border text-right">{{ formatAmount(getTotalLoanDue(item)) }}</td>
          </tr>
        </tbody>
      </table>
...
const getTotalLoanDue = (item) => {
  const loanAmount = Number(item?.LoanAmount || 0);
  const interestAmount = Number(item?.InterestAmount || 0);
  const totalPaid = Number(item?.TotalPayment || 0);
  return Math.max(loanAmount + interestAmount - totalPaid, 0);
};
...
const fetchData = async () => {
  ...
  try {
    ...
    totalLoanDue.value = rows.value.reduce((sum, item) => sum + getTotalLoanDue(item), 0);
  } catch (error) {
    console.error(error);
    showNotification("error", "Failed to fetch total loan due.");
  } finally {
    loading.value = false;
  }
};
