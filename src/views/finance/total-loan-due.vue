  ...
  <MainLayout>
    <div class="max-w-7xl mx-auto space-y-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h1 class="text-2xl font-bold text-indigo-700">{{ $t('loan.remaining') }}</h1>
      ...
      <div class="text-right mb-3 flex justify-end items-center gap-2">
        <label class="font-semibold text-gray-700">{{ $t('loan.remaining') }}</label>
      ...
      <table v-else class="w-full border border-collapse text-left">
        <thead>
          <tr class="bg-primary text-white">
            ...
            <th class="border border-white px-4 py-2 text-right">{{ $t('loan.totalLoanDue') }}</th>
          </tr>
        </thead>
        <tbody>
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error(error);
    showNotification("error", t("loan.fetchErrorDue"));
  } finally {
    loading.value = false;
  }
};
